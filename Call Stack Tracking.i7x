...
To say the location of (A - a call frame) with frame number (I - a number) (this is saying the location of each call frame):
	...
	if the call frame numbering flag is true and the terse call stack numbers flag is true:
	...
	otherwise:
		if the function address is not the actual function address:
			say " (a substitution for [the human-friendly name] at address [the function address])";
	if the call frame numbering flag is true and the terse call stack numbers flag is false:
		say " (call frame #[I converted to a number])".
...
and shorten the presentation of these numbers from text like " (call frame #9)"
to just "9." with

	now the terse call stack numbers flag is true;
